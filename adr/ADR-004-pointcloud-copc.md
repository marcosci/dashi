...
- **PoC-Implementierung:** `dashi-ingest` ruft PDAL (`writers.copc`) als Subprozess auf. Reprojektion (Quell-CRS → EPSG:4326) erfolgt im gleichen PDAL-Pipeline-Stage via `filters.reprojection`. PDAL-Binary muss auf PATH verfügbar sein (`brew install pdal` / `apt install pdal`) — fehlt PDAL, wird Pointcloud-Ingestion sauber übersprungen und in `IngestOutcome.reason` begründet.
