...
# Registers the dashi-ingest deployment, triggers a run, verifies the
...
SCH=$(curl -sf "http://localhost:${PREFECT_PORT}/api/deployments/name/dashi-ingest/main" | python3 -c 'import sys,json;s=json.load(sys.stdin).get("schedules",[]);print(s[0]["schedule"].get("cron") if s else "")')
[[ -n "$SCH" ]] && ok "deployment dashi-ingest/main has cron '$SCH'" || fail "no cron schedule on deployment"

# 3. Trigger a flow run
RUN_ID=$("$INGEST_VENV/bin/prefect" deployment run 'dashi-ingest/main' 2>&1 | grep -oE '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' | head -1)
...
# 5. Verify a pod with the flow-run-id label ran in dashi-data
...
HTTP=$(curl -s -o /dev/null -w '%{http_code}' "$DASHI_STAC_URL/collections/gelaende-umwelt/items?limit=1")
